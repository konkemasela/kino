...
export default function WatchExperience({
  ...
  const showSeasons = kind === "tv" && seasonNumbers.length > 0;
            ...
            {seasonEpisodes.map((ep) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group/ep overflow-hidden rounded-xl border text-left transition-all duration-300 active:scale-[0.98]",
                    active
                      ? "border-acid bg-acid/10 shadow-[0_0_28px_rgba(215,246,55,0.12)]"
                      : "border-line bg-panel/60 hover:border-bone/30",
                  )}
                >
                  <div className="relative aspect-video w-full overflow-hidden bg-panel2">
                    {ep.thumbnail ? (
                      <Image
                        ...
                        className="object-cover transition-transform duration-500 group-hover/ep:scale-105"
                      />
                    ) : (
                      <div className="flex h-full items-center justify-center font-display text-4xl text-outline">
                        {String(ep.episode).padStart(2, "0")}
                      </div>
                    )}
                    {active && (
                      <span className="absolute right-2 top-2 rounded-full bg-acid px-2.5 py-1 text-[9px] font-bold tracking-[0.14em] text-void uppercase">
                        Playing
                      </span>
                    )}
                  </div>
                  <div className="p-3">
                    <p className="flex items-center gap-2 text-[10px] font-bold tracking-[0.2em] text-acid uppercase">
                      E{String(ep.episode).padStart(2, "0")}
                      {ep.released ? (
                        <span className="text-fog">
                          · {ep.released.slice(0, 4)}
                        </span>
                      ) : null}
                    </p>
                    <p className="mt-1 truncate text-[13px] font-semibold text-bone/90">
                      {ep.name}
                    </p>
                  </div>
                </button>
