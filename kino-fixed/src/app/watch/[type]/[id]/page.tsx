...
async function AnimeWatch({
  ...
  const episodeList: EpisodeDto[] = episodesFrom(meta).map((e) => ({
    id: `${id}-${e.number}`,
    season: Math.max(1, Math.ceil(e.number / 12)),
    episode: e.number,
    name: e.title,
    overview: "",
    released: e.aired,
  ...
  if (!episodeList.some((e) => e.season === season && e.episode === episode)) {
    const fallback = episodeList.find((e) => e.episode === episode) ?? episodeList[0];
    season = fallback?.season ?? 1;
    episode = fallback?.episode ?? 1;
  }

  const current = episodeList.find(
    (e) => e.season === season && e.episode === episode,
  );
