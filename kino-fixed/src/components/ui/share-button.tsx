...
export default function ShareButton({
  ...
  useEffect(() => {
    ...
    const baseUrl = new URL(path, window.location.origin).toString();
    setUrl(baseUrl);
    ...
    const watchUrl = new URL(baseUrl);
    watchUrl.searchParams.set("party", "community");
    watchUrl.searchParams.set("id", party.id);
    watchUrl.searchParams.set("type", party.type);
    if (party.season !== undefined) watchUrl.searchParams.set("s", String(party.season));
    if (party.episode !== undefined) watchUrl.searchParams.set("e", String(party.episode));
    setPartyUrl(watchUrl.toString());
        ...
        {open && (
          ...
          >
            ...
            >
              ...
              {partyUrl && (
                <div className="mt-5 rounded-2xl border border-acid/30 bg-acid/5 p-3">
                  <p className="text-[10px] font-bold tracking-[0.24em] text-acid uppercase">
                    Community watch party
                  </p>
                  <div className="mt-3 flex items-center gap-3 rounded-2xl border border-line bg-void/50 p-2 pl-4">
                    <Link2 size={15} className="shrink-0 text-fog" />
                    <span className="flex-1 truncate text-[12px] text-bone/70">
                      {partyUrl}
                    </span>
                    <button
                      onClick={() => copyText(partyUrl)}
                      className="flex shrink-0 items-center gap-2 rounded-xl bg-acid px-3 py-2 text-[9px] font-bold tracking-[0.16em] uppercase text-void transition-all active:scale-95"
                    >
                      <Copy size={12} />
                      Copy
                    </button>
